...
- 商家中心现已拆分为 `工作台 / 商品管理 / 添加商品 / 店铺资料 / 地址管理` 五个 tab。
- 工作台统一承载订单与售后处理，切换订单状态或售后状态筛选时会自动刷新页面数据，并带局部加载动画。
- 工作台订单卡片中的商品改为点击后居中弹层清单，背景会轻微模糊，商家侧不再通过商品名跳转商品详情。
- 工作台里的搜索框、筛选框、复选框已统一为更完整的暖金控件风格。
- 商品管理支持搜索、单商品编辑、单商品上下架和多选批量下架；添加商品独立成单独 tab。
- 店铺资料与地址管理从原混排区域拆出；地址现在支持新增、编辑、设为默认和删除。
- 商品录入继续支持维护 `sku_code`，并兼容品牌、型号、评分、月销、标签、核心参数等比较字段。
...
- 购物车条目中的商品图片与标题支持直接跳转到商品详情页，便于从购物车回看规格与店铺信息。
