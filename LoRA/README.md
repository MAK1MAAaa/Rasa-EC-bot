...
| 运行批次 | 配置 | 训练样本数 | 验证样本数 | 测试样本数 | 最终损失 | 通过率 | 幻觉订单号 | 缺失确认 | 备注 |
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |
...
## 15. 2026-04-10 训练输入归一化更新

为支持 `data/processed/combined_react_agent/*.jsonl` 这类混合字段数据，`scripts/train_lora.py` 在构建数据集前会先对每条 JSONL 记录做归一化。

变更点：
- 加载器仅保留训练必需字段：`id`、`source`、`category`、`intent`、`messages`。
- 额外字段（例如 `action`、`generation_mode`、`origin_source`）会自动忽略。
- 非法行（JSON 格式错误或 `messages` 非法）会被跳过并计数。
- `run_summary.json` 新增统计字段：
  - `train_records_skipped_in_normalization`
  - `eval_records_skipped_in_normalization`

影响：
- 可直接使用混合数据集训练，不再因 schema 不一致触发 HuggingFace `CastError`。
- 现有纯 SFT 数据集保持兼容。
